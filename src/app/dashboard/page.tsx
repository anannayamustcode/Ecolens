...
export default function Dashboard() {
  ...
  useEffect(() => {
    console.log('Dashboard mounted with search params:', { // Debug log
      front: searchParams.get('front'),
    ...
    const fetchData = async () => {
      try {
        console.log('Fetching additional product data...'); // Debug log
        const response = await fetch(`http://localhost:5000/api/compare?front=${searchParams.get('front')}&back=${searchParams.get('back')}`);
        const data = await response.json();
        console.log('Product data received:', data); // Debug log
        
        // Process your data here
      } catch (error) {
        console.error('Error fetching product data:', error); // Error log
      }
    };

    if (searchParams.get('front') || searchParams.get('back')) {
      fetchData();
    }
  }, [searchParams]);
    ...
    <div>
      ...
      {searchParams.get('front') && (
        <div>
          <h2>Front Image</h2>
          <img 
            src={`http://localhost:5000/uploads/${searchParams.get('front')}`} 
            alt="Front view" 
            onError={(e) => console.error('Error loading front image:', e)} // Error log
          />
        </div>
      )}
      {searchParams.get('back') && (
        <div>
          <h2>Back Image</h2>
          <img 
            src={`http://localhost:5000/uploads/${searchParams.get('back')}`} 
            alt="Back view" 
            onError={(e) => console.error('Error loading back image:', e)} // Error log
          />
        </div>
      )}
    </div>
